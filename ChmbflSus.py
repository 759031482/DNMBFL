...
class Get_sus:
    ...
    def Random(self, data_json, times=1000):
        if not data_json:
            return False
        try:
            doc = list(data_json.keys())[0]
...
def CHMBFL():
    ...
    def CHMBFL_mbfl_baseline_res():
        ...
        for method, function in baseline_list:
            ...
            docpath = os.path.join(os.getcwd(), save_dirpath, 'baseline', method, 'max')
            if not os.path.exists(docpath):
                os.makedirs(docpath)
    ...
    def CHMBFL_sbfl_baseline_res():
        ...
        for method, function in functionlist:
            ...
            doc_rootpath = os.path.join(os.getcwd(), save_dirpath, 'baseline', method)
    ...
    def CHMBFL_timesable_repeatneed_res():
        ...
        for times in [0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4]:
            for parameter in clasterfunction:
                ...
                for repeat in range(1, 10):
                    ...
                    for wb_i, wb in enumerate(Wbs):
                        ...
                        docpath = os.path.join(os.getcwd(), save_dirpath, 'DNMbfl', method, word)
                        if not os.path.exists(docpath):
                            os.makedirs(docpath)
    ...
    def CHMBFL_baseline_timesable_repeatneed_res():
        ...
        # for times in [1, 2, 3, 5, 7, 10, 20, 50, 100]:
        for times in [0.5]:
            # for method, function in clasterfunction:
            for parameter in clasterfunction:
                if '.' in str(times):
                    strtimes = str(times).replace('.', '_')
                else:
                    strtimes = str(times)
                method = parameter[0] + '-%s' % strtimes
                function = parameter[1]
                for repeat in range(1, 10):
                    # 表格初始化
                    Wbs = [openpyxl.Workbook(), openpyxl.Workbook(), openpyxl.Workbook()]
                    ...
                    for wb_i, wb in enumerate(Wbs):
                        word = ['max', 'ave', 'frequency', 'none'][wb_i]
                        docpath = os.path.join(os.getcwd(), save_dirpath, 'HomBaseline', method, word)
                        if not os.path.exists(docpath):
                            os.makedirs(docpath)
                        filepath = os.path.join(docpath, '%s_%s_%sth_ranke.xlsx' % (method, word, repeat))
                        wb.save(filepath)
...
if __name__ == '__main__':
    print(data_dirpath, save_dirpath)
    CHMBFL()
    # getExample()
