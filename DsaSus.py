...
class Tools:
    ...
    def find_hom_data(self, hom_list_all, hom_message):
        # 在二阶变异体中寻找信息
        som_message_1 = hom_message[0]
        som_message_2 = hom_message[1]
        for j, hom in enumerate(hom_list_all):
            hom_message_1 = hom['message'][0]
            hom_message_2 = hom['message'][1]
            if hom_message_1 == som_message_1 and hom_message_2 == som_message_2:
                return hom
            if hom_message_1 == som_message_2 and hom_message_2 == som_message_1:
                return hom
...
class Get_sus:
    ...
    # 计算聚类怀疑度
    def get_sus_claster(self, data_json, method, claster_num):
        if not data_json:
            return False
        try:
            doc = list(data_json.keys())[0]
        ...
        # 进行聚类
        classes = Cluster_Fom.cluster_fom(fom_list, method, claster_num)

        # ------------------------------------------------
        # 生成som组合
        som_list = []
        for label in classes.keys():
            for i, fom_i in enumerate(classes[label]):
                for j, fom_j in enumerate(classes[label]):
                    if fom_i[0] >= fom_j[0]:
                        continue
                    som = util.Datasave().hom
                    som['message'].append(fom_i)
                    som['message'].append(fom_j)
                    som_list.append(som)

        # ------------------------------------------------
        # 获取som信息
        for i, som in enumerate(som_list):
            hom = Tools().find_hom_data(hom_list_all, som['message'])
            if not hom:
                continue
            som_list[i]['spectrum'] = hom['spectrum']
            som_list[i]['out_list'] = hom['out_list']
            som_list[i]['out_or'] = hom['out_or']
            som_list[i]['time'] = hom['time']
        ...
        for hom in som_list:
            ...
            hom_out_or = hom["out_or"]
            hom_out_list = hom["out_list"]
            totaltime += hom["time"]
            for fom in hom_message:
                if fom[0] not in hom_dic:
                    hom_dic[fom[0]] = []
                hom_dic[fom[0]].append(hom_out_or)


        # ------------------------------------------------
        # 计算怀疑度
        touple = mbfl.command.get_toule(true_out, or_out, hom_dic)
        for mbfl_for in mbfl_for_list:
    ...
    # 计算mbfl方法怀疑度
    def get_sus_mbfl(self, data_json):
        if not data_json:
            return False
        try:
            doc = list(data_json.keys())[0]
    ...
    # 计算sbfl方法怀疑度
    def get_sus_sbfl(self, data_json):
        if not data_json:
            return False
        try:
            doc = list(data_json.keys())[0]
