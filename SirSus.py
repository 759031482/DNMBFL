...
class Tools:
    ...
    def sir_rules(self, path, firstread=True, highread=True, covread=True):
        ...
        if firstread:
            ...
            for i in range(len(fommessagelist)):
                ...
                fom = line.split('<:>')
                fom_list.append({'message': [[int(fom[2]), '', '', fom[4],  fom[5], int(fom[3])]],
                                 "out_list": out_list, "kill_list": kill_list})
        ...
        if highread:
            ...
            for i in range(len(fommessagelist)):
                ...
                hom_out_list.append({'message': fom, "out_list": out_list, "kill_list": kill_list})
...
class Get_sus:
    ...
    # 基于变异体聚类
    def MutCluster(self, data_json, seting, times=10000):
        if not data_json:
            return False
        try:
            doc = list(data_json.keys())[0]
    ...
    def ErrorDistribution(self, data_json, seting, times=10000):
        if not data_json:
            return False
        try:
            doc = list(data_json.keys())[0]
    ...
    def NS(self, data_json, seting, times=1000):
        if not data_json:
            return False
        try:
            doc = list(data_json.keys())[0]
...
def CHMBFL():
    ...
    def CHMBFL_mbfl_baseline_res():
        ...
        for method, function in baseline_list:
            ...
            docpath = os.path.join(os.getcwd(), 'report/CHMBFL/susfile-sir/baseline/', method, 'max')
            ...
            # 获取数据
            for file_i, [project, faultnum, version] in enumerate(Tools().version_path()):
                ...
                sus_data_json = function(Tools().sir_rules(path, highread=False))
    ...
    def CHMBFL_sbfl_baseline_res():
        ...
        for method, function in functionlist:
            ...
            doc_rootpath = os.path.join(os.getcwd(), 'report/CHMBFL/susfile-sir/baseline/', method)
            ...
            for file_i, [project, faultnum, version] in enumerate(Tools().version_path()):
                ...
                # 计算相应方法的怀疑度
                path = os.path.join(data_dirpath, project, faultnum, version)
                sus_data_json = function(Tools().sir_rules(path, highread=False, firstread=False))
            ...
            for parameter in clasterfunction:
                ...
                for repeat in range(1, 40):
                    ...
                    for file_i, project, faultnum, version in enumerate(Tools().version_path()):
                        ...
                        if len(parameter) == 2:
                            sus_data_json = function(Tools().sir_rules(path), times)
                        else:
                            sus_data_json = function(Tools().sir_rules(path), parameter[2], times)
                    ...
                    for wb_i, wb in enumerate(Wbs):
                        ...
                        docpath = os.path.join(os.getcwd(), 'report/CHMBFL/susfile-sir/DNMbfl/', method, word)
                        if not os.path.exists(docpath):
                            os.makedirs(docpath)
            ...
            for parameter in clasterfunction:
                ...
                for repeat in range(1, 10):
                    ...
                    for file_i, [project, faultnum, version] in enumerate(Tools().version_path()):
                        ...
                        path = os.path.join(data_dirpath, project, faultnum, version)
                        if times == 1000:
                            sus_data_json = function(Tools().sir_rules(path))
                        else:
                            sus_data_json = function(Tools().sir_rules(path), times)
                    ...
                    for wb_i, wb in enumerate(Wbs):
                        word = ['max', 'ave', 'frequency', 'none'][wb_i]
                        docpath = os.path.join(os.getcwd(), 'report/CHMBFL/susfile-sir/HomBaseline', method, word)
                        if not os.path.exists(docpath):
                            os.makedirs(docpath)
                        filepath = os.path.join(docpath, '%s_%s_%sth_ranke.xlsx' % (method, word, repeat))
                        wb.save(filepath)
    ...
    # 不需要倍数，需要重复次数
    def CHMBFL_baseline_timesable_repeatneed_res_new():
        clasterfunction = [
            ['Last2First', Get_sus().Last2First],
            ['DifferentOperator', Get_sus().DifferentOperator],
            ['RandomMix', Get_sus().RandomMix],
        ...
        for times in [0.5]:
            ...
            for parameter in clasterfunction:
                ...
                # 存储文件
                def getrestrepeat(repeattimes, method):
                    for repeat in range(1, repeattimes+1):
                        for word in ['max', 'ave', 'frequency', 'none']:
                            ...
                            if not os.path.exists(filepath):
                                return repeat
                if continuecreat:
                    restrepeat = getrestrepeat(repeattimes, method)
                else:
                    restrepeat = 1
                ...
                for file_i, [project, faultnum, version] in enumerate(Tools().version_path()):
                    file = '%s-%s-%s' % (project, faultnum, version)
                    path = os.path.join(data_dirpath, project, faultnum, version)
                    sir_data = Tools().sir_rules(path, covread=False, firstread=False)
                    for repeat in range(restrepeat, repeattimes+1):
                        print('%s method:%s, repeat:%s read:%s - %s' %
                              (datetime.datetime.now(), method, repeat, file, file_i))
                        # 计算相应方法的怀疑度
                        if times == 1000:
                            sus_data_json = function(sir_data)
                        else:
                            sus_data_json = function(sir_data, times)
                        Wbs = RWbs[repeat-1]
                        for wb_i, wb_name in enumerate(['max', 'ave', 'frequency']):
                            wb = Wbs[wb_i]
                            word = ['max', 'ave', 'frequency'][wb_i]
                            for ws_name in ['best', 'average', 'worst']:
                                ws = wb['exam-%s' % ws_name]
                                sheetdata_list = [
                                    str(file_i+1),
                                    file,
                                ]
                                ...
                                for mbfl_for_j, mbfl_for in enumerate(mbfl_for_list):
                                    ...
                                    ranklist = []
                                    for fault_ith in range(maxfaultnum):
                                        if fault_ith > len(sus_data_json[word][mbfl_for]['rank_%s' % ws_name])-1:
                                            examlist.append('')
                                            ranklist.append('')
                                        else:
                                            examlist.append(sus_data_json[word][mbfl_for]['exam_%s' % ws_name][fault_ith])
                                            ranklist.append(sus_data_json[word][mbfl_for]['rank_%s' % ws_name][fault_ith])
                                    sheetdata_list += examlist
                                    sheetdata_list += ranklist
                                for sheetdata_i, sheetdata in enumerate(sheetdata_list):
                                    ws.cell(file_i + 3, sheetdata_i+1, sheetdata)
    ...
    # 不需要倍数，需要重复次数
    def CHMBFL_timesable_repeatneed_res_new():
        clasterfunction = [
            ['DeltaNsMbfl', Get_sus().DeltaNsMbfl],
        ]
        ftitle = [
    ...
    # CHMBFL_timesable_repeatneed_res()

    # CHMBFL_baseline_timesable_repeatneed_res_new()
    # CHMBFL_timesable_repeatneed_res_new()
...
def path_test():
    for [project, faultnum, version] in Tools().version_path():
        path = os.path.join(data_dirpath, project, faultnum, version)
        ...
        for file in os.listdir(os.path.join(path, 'defect_root', 'source')):
            if '.c' not in file:
                continue
            filepath_list.append(os.path.join(path, 'defect_root', 'source', file))
