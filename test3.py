...
class Get_sus:
    ...
    # 基于变异体聚类
    def MutCluster(self, data_json, times=10000):
        if not data_json:
            return False
        try:
            doc = list(data_json.keys())[0]
    ...
    # 基于失败测试用例
    def FailTestCluster(self, data_json, times=10000):
        if not data_json:
            return False
        try:
            doc = list(data_json.keys())[0]
    ...
    # mbfl指导基于错误分布
    def ErrorDistribution(self, data_json, times):
        # print('%s 读取数据' % datetime.datetime.now())
        if not data_json:
            return False
        try:
    ...
    # mbfl指导基于错误分布
    def ED_SBFL(self, data_json, times):
        # print('%s 读取数据' % datetime.datetime.now())
        if not data_json:
            return False
        try:
    ...
    def NS(self, data_json, times=1000):
        if not data_json:
            return False
        try:
            doc = list(data_json.keys())[0]
...
def CHMBFL():
    ...
    def CHMBFL_hmbfl_baseline_res():
        repeat_times = 3
        baseline_list = [['Mbfl', Get_sus().get_sus_last2first], ]
        for wb_name, baseline in baseline_list:
            for repeat in range(1, repeat_times+1):
                ...
                for i, file in enumerate(os.listdir(data_dirpath)):
                    print('%s baseline:%s, read:%s' % (datetime.datetime.now(), wb_name, file))
                    ...
                    for j, mbfl_for in enumerate(mbfl_for_list):
                        for sheet, name in sheetlist:
                            ...
                            for fault_ith in range(len(sus_data_json['Fault_Record'])):
                                ...
                                rank = sus_data_json[mbfl_for]['rank_%s' % name][fault_ith]
                                exam = sus_data_json[mbfl_for]['exam_%s' % name][fault_ith]

                                sheet.cell(i + 3, 5+fault_ith + j*6, exam)
                                sheet.cell(i + 3, 5+fault_ith+3 + j*6, rank)

                doc_rootpath = os.path.join(os.getcwd(), 'report', 'CHMBFL', 'susfile', 'baseline')
                if wb_name == 'RandomMix':
                    docpath = os.path.join(doc_rootpath, 'RandomMix', mbfl_sustype)
                else:
                    docpath = os.path.join(doc_rootpath, wb_name, mbfl_sustype)
                if not os.path.exists(docpath):
                    os.makedirs(docpath)

                if wb_name == 'RandomMix':
                    filepath = os.path.join(docpath, '%sth_%s_%s_rank.xlsx' % (repeat, wb_name, mbfl_sustype))
                else:
                    filepath = os.path.join(docpath, '%s_%s_rank.xlsx' % (wb_name, mbfl_sustype))
                wb.save(filepath)
    ...
    def CHMBFL_timesable_repeatneed_res():
        ...
        if True:
            clasterfunction += [
                ...
                # ['ED.SBFL', Get_sus().ED_SBFL],
                # ['RegroupBySus-H', Get_sus().RegroupBySus],
                # ['RegroupBySus-New', Get_sus().NewRegroupBySus],
                # ['FailTestCluster', Get_sus().FailTestCluster],
                # ['NS', Get_sus().NS],
                # ['NS-MBFL', Get_sus().NS],
                # ['NS-SBFL', Get_sus().NS],
        ...
        if True:
            clasterfunction += [
                # ['NS-MutCluster-DifClass', Get_sus().MutCluster],
                # ['NS-MutCluster-DifClass (10', Get_sus().MutCluster],
                # ['NS-ErrorDistribution', Get_sus().ErrorDistribution],
                # ['NS-RegroupBySus-H', Get_sus().RegroupBySus],
                # ['NS-FailTestCluster', Get_sus().FailTestCluster],
        ...
        if True:
            clasterfunction += [
                # ['NS-RS-FTC', Get_sus().RS_FTC],
                # ['NS-RS-MC', Get_sus().MC_RS],
                # ['NS-RS-ED', Get_sus().RS_ED],
                # ['NS-RS-MC+ED', Get_sus().MC_RS_ED],
                # ['NS-RS-MC+FTC', Get_sus().MC_RS_FTC],

                # ['NS-MC+ED', Get_sus().MC_ED],
                # ['NS-MC+FTC', Get_sus().MC_FTC],
                # ['NS-MC+RS', Get_sus().MC_RS],
                # ['NS-RS+ED', Get_sus().RS_ED],
                # ['NS-RS+FTC', Get_sus().RS_FTC],
                # ['NS-MC+RS+ED', Get_sus().MC_RS_ED],
                # ['NS-MC+RS+FTC', Get_sus().MC_RS_FTC],
            ]
            clasterfunction += [
                # ['RS-MC+FTC', Get_sus().MC_RS_FTC],
                # ['RS-MC+ED', Get_sus().MC_RS_ED],
                # ['MC+RS+ED', Get_sus().MC_RS_ED],
            ]
        ...
        for method, function in clasterfunction:
            for repeat in range(1, 20):
                ...
                for wb_i, wb in enumerate(Wbs):
                    word = ['max', 'ave', 'frequency'][wb_i]
                    docpath = os.path.join(os.getcwd(), 'report/CHMBFL/susfile/numless', method, word)
                    if not os.path.exists(docpath):
                        os.makedirs(docpath)
